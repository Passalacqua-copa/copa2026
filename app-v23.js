...
/* ===== FIX FINAL MATA-MATA V23 =====
   Bloco único para:
   - exibir botão de placar no mata-mata em Admin;
   - abrir modal;
   - salvar placar dos jogos 73 a 104;
   - manter pênaltis para jogos empatados.
*/

function isAdminActiveV23() {
  return isAdmin === true || sessionStorage.getItem(ADMIN_KEY) === '1';
}

function matchHtmlKoV23(idx, m, a, b) {
  var sc = m.time;
  var st = '';
  var openScore = ' open';
  ...
  var edit = isAdminActiveV23()
    ? '<button class="edit ko-edit" type="button" onclick="openMatch(' + idx + ')">⚽ placar</button>'
    : '';
...
renderKnockout = function() {
  ...
  for (var i = 0; i < matches.length; i++) {
    ...
    html += matchHtmlKoV23(i, m, nm.a, nm.b);
  }
...
function safeSetHtmlV23(id, value) {
  var el = document.getElementById(id);
  if (el) el.innerHTML = value == null ? '' : value;
}

function safeSetValueV23(id, value) {
  var el = document.getElementById(id);
...
openMatch = function(idx) {
  ...
  safeSetHtmlV23('mtitle', title);
  safeSetHtmlV23('fa', isRealTeam(names.a) ? flagImg(names.a, 'big') : '');
  safeSetHtmlV23('fb', isRealTeam(names.b) ? flagImg(names.b, 'big') : '');
  safeSetHtmlV23('na', names.a);
  safeSetHtmlV23('nb', names.b);

  safeSetValueV23('ga', m.gA);
  safeSetValueV23('gb', m.gB);
  safeSetValueV23('pa', m.pA);
  safeSetValueV23('pb', m.pB);
...
function saveRemoteResultV23(m) {
  return new Promise(function(resolve, reject) {
    var cbName = 'copaSaveCallbackV23_' + Date.now();
...
saveMatch = function() {
  ...
  saveRemoteResultV23(m).then(function() {
    toast('Resultado salvo na planilha.');
    setTimeout(loadRemoteResults, 800);
  }).catch(function(err) {
    console.error(err);
...
var originalUpdateAdminUIV23 = updateAdminUI;
updateAdminUI = function() {
  originalUpdateAdminUIV23();
