...
/* ===== V32 SINCRONIZAÇÃO FINAL ===== */

function nRemote(v){ if(v==null || v==='') return null; var n=Number(v); return isNaN(n)?null:n; }

applyRemoteResults = function(results){
  for(var i=0;i<matches.length;i++){
    var m=matches[i], r=results[m.num] || results[String(m.num)];
    if(!r) continue;
    m.gA=nRemote(r.gA);
    m.gB=nRemote(r.gB);
    m.status=r.status || 'agendado';
    var pa = r.pA; if(pa==null) pa=r.penA;
    var pb = r.pB; if(pb==null) pb=r.penB;
    m.pA=nRemote(pa);
    m.pB=nRemote(pb);
  }
  persist();
...
saveRemoteResult = function(m){
  return new Promise(function(resolve,reject){
    var cb='copaSaveCallbackV32_'+Date.now();
    window[cb]=function(data){
      try{ if(data && data.ok) resolve(data); else reject(data || {error:'Falha ao salvar'}); }
      finally{ delete window[cb]; var el=document.getElementById(cb); if(el&&el.parentNode)el.parentNode.removeChild(el); }
    };
    var params=[
      'action=save',
      ...
      'golsB='+encodeURIComponent(m.gB==null?'':String(m.gB)),
      'status='+encodeURIComponent(m.status||'agendado'),
      'pA='+encodeURIComponent(m.pA==null?'':String(m.pA)),
      'pB='+encodeURIComponent(m.pB==null?'':String(m.pB)),
      'penA='+encodeURIComponent(m.pA==null?'':String(m.pA)),
      'penB='+encodeURIComponent(m.pB==null?'':String(m.pB)),
      't='+Date.now()
    ].join('&');
    var s=document.createElement('script');
    s.id=cb; s.src=SHEETS_API_URL+'?'+params;
    s.onerror=function(){ delete window[cb]; reject({error:'Erro de comunicação'}); };
    document.body.appendChild(s);
  });
};

loadRemoteResults = function(){
  var cb='copaResultsCallbackV32_'+Date.now();
  window[cb]=function(data){
    try{
      if(data && data.ok && data.results){
        applyRemoteResults(data.results);
      ...
      delete window[cb]; var el=document.getElementById(cb); if(el&&el.parentNode)el.parentNode.removeChild(el);
    }
  };
  var s=document.createElement('script');
  s.id=cb; s.src=SHEETS_API_URL+'?callback='+encodeURIComponent(cb)+'&t='+Date.now();
  s.onerror=function(){ delete window[cb]; toast('Não foi possível sincronizar a planilha.'); };
  document.body.appendChild(s);
};
...
function refName(ref, stack){
  ...
  stack[key]=true;
  var m=findMatch(num);
  if(!m || m.status!=='encerrado' || m.gA==null || m.gB==null) return type==='W'?'Vencedor Jogo '+num:'Perdedor Jogo '+num;
  var names=matchNames(m, stack);
  var winner=null, loser=null;
  if(m.gA>m.gB){winner=names.a; loser=names.b;}
  else if(m.gB>m.gA){winner=names.b; loser=names.a;}
  else if(m.type==='ko' && m.pA!=null && m.pB!=null && m.pA!==m.pB){
    if(m.pA>m.pB){winner=names.a; loser=names.b;} else {winner=names.b; loser=names.a;}
  }
  return type==='W' ? (winner || 'Vencedor Jogo '+num) : (loser || 'Perdedor Jogo '+num);
}
function winnerOf(num){ var n=refName('W'+num,{}); return n.indexOf('Vencedor Jogo ')===0?null:n; }
function loserOf(num){ var n=refName('L'+num,{}); return n.indexOf('Perdedor Jogo ')===0?null:n; }
function resolveRef(ref){ return refName(ref,{}); }
