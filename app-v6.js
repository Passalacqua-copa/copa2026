...
function flagEmoji(name) {
  var code = flagCode(name);

  if (!code) return '';
  if (code === 'gb-eng' || code === 'gb-sct') return '🏴';
  if (code.length !== 2) return '';

  var first = code.toUpperCase().charCodeAt(0) - 65 + 0x1F1E6;
  var second = code.toUpperCase().charCodeAt(1) - 65 + 0x1F1E6;

  return String.fromCodePoint(first) + String.fromCodePoint(second);
}

function summaryName(name) {
  var e = flagEmoji(name);
...
function parseMatchDateKey(dateKey) {
...
function prettyDate(dateKey) {
  var d = parseMatchDateKey(dateKey);
  var dias = ['domingo','segunda-feira','terça-feira','quarta-feira','quinta-feira','sexta-feira','sábado'];
  return dateKey + ' (' + dias[d.getDay()] + ')';
}

function endedMatchesByDate(dateKey) {
  var arr = [];

  for (var i = 0; i < matches.length; i++) {
    var m = matches[i];

    if (m.date === dateKey && m.status === 'encerrado' && m.gA != null && m.gB != null) {
      arr.push(m);
    }
  }
...
function nextBrazilMatchText() {
  for (var i = 0; i < matches.length; i++) {
    var m = matches[i];
    var a = m.type === 'ko' ? knockoutNames()[m.num].a : m.a;
    var b = m.type === 'ko' ? knockoutNames()[m.num].b : m.b;

    if ((a === 'Brasil' || b === 'Brasil') && m.status !== 'encerrado') {
      return '🇧🇷 Próximo jogo do Brasil\n' +
        summaryName(a) + ' x ' + summaryName(b) + '\n' +
        m.date + ' • ' + m.time + ' • ' + m.city;
    }
  }

  return '🇧🇷 Próximo jogo do Brasil\nAinda não definido ou fase já encerrada.';
}

function formatResultLine(m) {
  var a = m.type === 'ko' ? knockoutNames()[m.num].a : m.a;
  var b = m.type === 'ko' ? knockoutNames()[m.num].b : m.b;
  var line = summaryName(a) + ' ' + m.gA + ' x ' + m.gB + ' ' + summaryName(b);

  if (m.type === 'ko' && m.gA === m.gB && m.pA != null && m.pB != null) {
    line += ' (' + m.pA + ' x ' + m.pB + ' pen.)';
  }

  return line;
}

function impactedGroupsFromMatches(dayMatches) {
  var map = {};

  for (var i = 0; i < dayMatches.length; i++) {
    var m = dayMatches[i];
    if (m.type === 'group' && m.group) map[m.group] = true;
  }

  return Object.keys(map).sort();
}

function groupSummaryText(groupId, tabs) {
  var lines = [];
  lines.push('Grupo ' + groupId);

  for (var i = 0; i < tabs[groupId].length; i++) {
    var t = tabs[groupId][i];
    lines.push((i + 1) + 'º ' + summaryName(t.team) + ' — ' + t.pts + ' pts | SG ' + gd(t));
  }
...
function copySummary() {
  var targetDate = yesterdayDateKey();
  var dayMatches = endedMatchesByDate(targetDate);
  var tabs = groupTables();
  var txt = '🏆 COPA DO MUNDO 2026 — PASSALACQUA\n\n';
  txt += '📅 Resumo dos jogos de ontem — ' + prettyDate(targetDate) + '\n\n';
  ...
  if (dayMatches.length === 0) {
    ...
    for (var i = 0; i < dayMatches.length; i++) {
      txt += formatResultLine(dayMatches[i]) + '\n';
    }

    var impacted = impactedGroupsFromMatches(dayMatches);

    if (impacted.length > 0) {
      txt += '\n📊 Classificação atualizada\n\n';

      for (var j = 0; j < impacted.length; j++) {
        txt += groupSummaryText(impacted[j], tabs) + '\n\n';
      }
    }
  }

  txt += nextBrazilMatchText() + '\n\n';
  txt += '🔗 Tabela completa:\n';
  txt += 'https://passalacqua-copa.github.io/copa2026/';

  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(txt).then(function() {
      toast('Resumo do dia anterior copiado.');
    }, function() {
      alert(txt);
    });
  } else {
