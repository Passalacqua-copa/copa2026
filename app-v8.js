...
function saveRemoteResult(m) {
  ...
  form.append('token', DATA.adminPassword);
  form.append('jogo', String(m.num));
  form.append('golsA', m.gA == null ? '' : String(m.gA));
  form.append('golsB', m.gB == null ? '' : String(m.gB));
  form.append('status', m.status || 'agendado');
...
function renderAll() { renderBrazilBox(); renderClass(); renderGames(); renderKnockout(); }
