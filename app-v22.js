...
/* ===== CORREÇÃO SALVAR MATA-MATA V19 ===== */

function saveRemoteResultV19(m) {
  return new Promise(function(resolve, reject) {
    var cbName = 'copaSaveCallbackV19_' + Date.now();
...
saveMatch = function() {
  ...
  saveRemoteResultV19(m).then(function() {
    toast('Resultado salvo na planilha.');
    setTimeout(loadRemoteResults, 800);
  }).catch(function(err) {
    console.error(err);
...
/* ===== CORREÇÃO BOTÃO PLACAR MATA-MATA V20 =====
   Sobrescreve a montagem visual dos jogos para garantir que o botão de placar
   apareça também nos jogos do mata-mata quando o usuário estiver em modo Admin.
*/
...
function matchHtml(idx, m, a, b) {
  ...
  var edit = isAdmin
    ? '<button class="edit" onclick="openMatch(' + idx + ')">⚽ placar</button>'
    : '';
...
/* Re-renderiza o mata-mata usando a função corrigida acima */
function renderKnockout() {
  var names = knockoutNames();
  var html = '';
  var current = '';
  ...
  html += '</div><div class="notice">No mata-mata, os vencedores avançam automaticamente para as próximas fases.</div>';

  document.getElementById('mata').innerHTML = html;
}


/* ===== CORREÇÃO DEFINITIVA BOTÃO MATA-MATA V21 =====
   Renderização própria da aba Mata-mata.
   O botão aparece se:
   - isAdmin === true
   - ou sessionStorage admin === '1'
*/

function isAdminActiveV21() {
  return isAdmin === true || sessionStorage.getItem(ADMIN_KEY) === '1';
}

function matchHtmlKoV21(idx, m, a, b) {
  var sc = m.time;
  var st = '';
  var openScore = ' open';
  ...
  var edit = isAdminActiveV21()
    ? '<button class="edit ko-edit" onclick="openMatch(' + idx + ')">⚽ placar</button>'
    : '';

  return '<div class="match ko-match' + brazil + '">' +
    '<span class="num">J' + m.num + '</span>' +
    '<div class="matchbody">' +
      '<div class="team-game">' + teamLabel(a, true) + '</div>' +
      '<span class="score' + openScore + '">' + sc + '</span>' +
...
renderKnockout = function() {
  ...
  for (var i = 0; i < matches.length; i++) {
    ...
    var nm = names[m.num] || { a: m.aRef || '', b: m.bRef || '' };
    html += matchHtmlKoV21(i, m, nm.a, nm.b);
  }
...
/* Garante que ao entrar/sair do admin a aba mata-mata seja redesenhada */
var originalUpdateAdminUIV21 = updateAdminUI;
updateAdminUI = function() {
  originalUpdateAdminUIV21();
  try { renderKnockout(); } catch(e) {}
};


/* ===== CORREÇÃO MODAL MATA-MATA V22 =====
   Sobrescreve openMatch para abrir corretamente jogos de grupos e mata-mata.
*/

function safeSetHtmlV22(id, value) {
  var el = document.getElementById(id);
  if (el) el.innerHTML = value == null ? '' : value;
}

function safeSetValueV22(id, value) {
  var el = document.getElementById(id);
  if (el) el.value = value == null ? '' : value;
}

openMatch = function(idx) {
  if (!isAdminActiveV21 || !isAdminActiveV21()) {
  ...
  var title = 'Jogo ' + m.num + ' · ' + (m.stage || ('Grupo ' + (m.group || ''))) + ' · ' + m.date + ' · ' + m.time;

  safeSetHtmlV22('mtitle', title);
  safeSetHtmlV22('fa', isRealTeam(names.a) ? flagImg(names.a, 'big') : '');
  safeSetHtmlV22('fb', isRealTeam(names.b) ? flagImg(names.b, 'big') : '');
  safeSetHtmlV22('na', names.a);
  safeSetHtmlV22('nb', names.b);

  safeSetValueV22('ga', m.gA);
  safeSetValueV22('gb', m.gB);
  safeSetValueV22('pa', m.pA);
  safeSetValueV22('pb', m.pB);

  var penbox = document.getElementById('penbox');
  if (penbox) {
    penbox.style.display = m.type === 'ko' ? 'flex' : 'none';
  }
