...
function matchHtml(idx, m, a, b) {
  ...
  var edit = adminOn
    ? '<button class="edit ko-open-v26" type="button" data-idx="' + idx + '">⚽ placar</button>'
    : '';
...
/* ===== MODAL SEGURO V26 =====
   Abre o modal por listener global, sem depender de onclick inline.
*/

function getKoDisplayNamesV26(m) {
  if (m.type === 'ko') {
    var names = knockoutNames();
    return names[m.num] || { a: m.aRef || '', b: m.bRef || '' };
  }
  return { a: m.a, b: m.b };
}

function openMatchSafeV26(idx) {
  var adminOn = isAdmin === true || sessionStorage.getItem(ADMIN_KEY) === '1';
  ...
  if (isNaN(idx) || idx < 0 || !matches[idx]) {
    alert('Jogo não encontrado.');
    return;
  }
  ...
  editIndex = idx;
  var m = matches[idx];
  var nm = getKoDisplayNamesV26(m);

  var titleEl = document.getElementById('mtitle');
  var faEl = document.getElementById('fa');
  var fbEl = document.getElementById('fb');
  var naEl = document.getElementById('na');
  var nbEl = document.getElementById('nb');
  var gaEl = document.getElementById('ga');
  var gbEl = document.getElementById('gb');
  var paEl = document.getElementById('pa');
  var pbEl = document.getElementById('pb');
  var penbox = document.getElementById('penbox');
  var overlay = document.getElementById('overlay');

  if (!overlay || !gaEl || !gbEl) {
    alert('Erro ao abrir o formulário de placar.');
    return;
  }

  if (titleEl) titleEl.innerHTML = 'Jogo ' + m.num + ' · ' + (m.stage || ('Grupo ' + (m.group || ''))) + ' · ' + m.date + ' · ' + m.time;
  if (faEl) faEl.innerHTML = isRealTeam(nm.a) ? flagImg(nm.a, 'big') : '';
  if (fbEl) fbEl.innerHTML = isRealTeam(nm.b) ? flagImg(nm.b, 'big') : '';
  if (naEl) naEl.innerHTML = nm.a;
  if (nbEl) nbEl.innerHTML = nm.b;

  gaEl.value = m.gA == null ? '' : m.gA;
  gbEl.value = m.gB == null ? '' : m.gB;
  if (paEl) paEl.value = m.pA == null ? '' : m.pA;
  if (pbEl) pbEl.value = m.pB == null ? '' : m.pB;

  if (penbox) penbox.style.display = m.type === 'ko' ? 'flex' : 'none';

  curStatus = m.status || 'agendado';
  if (typeof setStatus === 'function') setStatus(curStatus);

  overlay.className = 'overlay open';
}

/* Mantém compatibilidade com qualquer chamada antiga */
openMatch = openMatchSafeV26;

/* Listener delegado: funciona mesmo quando o botão é recriado no render */
document.addEventListener('click', function(ev) {
  var btn = ev.target && ev.target.closest ? ev.target.closest('.ko-open-v26, .edit') : null;

  if (!btn) return;

  var idx = btn.getAttribute('data-idx');

  /* Se for botão antigo com onclick inline, deixa o onclick atuar. */
  if (idx == null && btn.getAttribute('onclick')) return;

  if (idx == null) return;

  ev.preventDefault();
  ev.stopPropagation();
  openMatchSafeV26(idx);
}, true);
