...
/* ===== V27 SAFE KNOCKOUT + MODAL ===== */

function getMatchByNum(num) {
  for (var i = 0; i < matches.length; i++) if (matches[i].num === num) return matches[i];
  return null;
}
...
function resolveRef(ref, seen) {
  ...
  if (ref.charAt(0) !== 'W' && ref.charAt(0) !== 'L') return ref;

  var type = ref.charAt(0);
  var num = parseInt(ref.substring(1), 10);
  var key = type + num;
  if (seen[key]) return (type === 'W' ? 'Vencedor Jogo ' : 'Perdedor Jogo ') + num;
  seen[key] = true;

  var r = type === 'W' ? winnerOf(num, seen) : loserOf(num, seen);
  return r || (type === 'W' ? 'Vencedor Jogo ' : 'Perdedor Jogo ') + num;
}
...
function winnerOf(num, seen) {
  seen = seen || {};
  if (seen['W' + num]) return null;
  seen['W' + num] = true;

  var m = getMatchByNum(num);
  if (!m || m.status !== 'encerrado' || m.gA == null || m.gB == null) return null;

  var a = m.a, b = m.b;
  if (m.type === 'ko') {
    a = resolveRef(m.aRef, Object.assign({}, seen));
    b = resolveRef(m.bRef, Object.assign({}, seen));
  }

  if (m.gA > m.gB) return a;
  if (m.gB > m.gA) return b;
  if (m.type === 'ko' && m.pA != null && m.pB != null) {
    if (m.pA > m.pB) return a;
    if (m.pB > m.pA) return b;
  }
  return null;
}

function loserOf(num, seen) {
  seen = seen || {};
  if (seen['L' + num]) return null;
  seen['L' + num] = true;
  ...
  if (m.gA > m.gB) return b;
  if (m.gB > m.gA) return a;
  if (m.type === 'ko' && m.pA != null && m.pB != null) {
    if (m.pA > m.pB) return b;
    if (m.pB > m.pA) return a;
  }
  return null;
}
...
function matchHtml(idx, m, a, b) {
  ...
  if (m.status === 'encerrado' && m.gA != null && m.gB != null) {
    sc = m.gA + ' × ' + m.gB;
    if (m.type === 'ko' && m.gA === m.gB && m.pA != null && m.pB != null) sc += ' <small>(' + m.pA + '×' + m.pB + ' pen.)</small>';
    st = '<span class="status">ENC.</span>'; openScore = '';
  } else if (m.status === 'ao_vivo') {
    sc = (m.gA != null && m.gB != null) ? m.gA + ' × ' + m.gB : '? × ?';
    st = '<span class="status live">AO VIVO</span>'; openScore = '';
  }
  var gr = m.group ? '<span class="num">J' + m.num + ' · G' + m.group + '</span>' : '<span class="num">J' + m.num + '</span>';
  var brazil = (a === 'Brasil' || b === 'Brasil') ? ' brazil' : '';
  var adminOn = isAdmin === true || sessionStorage.getItem(ADMIN_KEY) === '1';
  var edit = adminOn ? '<button class="edit" type="button" onclick="openMatch(' + idx + ')">⚽ placar</button>' : '';
  return '<div class="match' + brazil + '">' + gr + '<div class="matchbody"><div class="team-game">' + teamLabel(a, true) + '</div><span class="score' + openScore + '">' + sc + '</span><div class="team-game">' + teamLabel(b, true) + '</div></div>' + st + '<span class="meta">' + m.date + ' · ' + m.city + '</span>' + edit + '</div>';
}

function openMatch(idx) {
  var adminOn = isAdmin === true || sessionStorage.getItem(ADMIN_KEY) === '1';
  if (!adminOn) { alert('Somente administrador pode alterar placares.'); return; }
  if (idx == null || idx < 0 || !matches[idx]) { alert('Jogo não encontrado.'); return; }
  ...
  document.getElementById('fa').innerHTML = isRealTeam(a) ? flagImg(a, 'big') : '';
  document.getElementById('fb').innerHTML = isRealTeam(b) ? flagImg(b, 'big') : '';
  document.getElementById('na').innerHTML = a;
  document.getElementById('nb').innerHTML = b;
  document.getElementById('ga').value = m.gA == null ? '' : m.gA;
  document.getElementById('gb').value = m.gB == null ? '' : m.gB;
  if (document.getElementById('pa')) document.getElementById('pa').value = m.pA == null ? '' : m.pA;
  if (document.getElementById('pb')) document.getElementById('pb').value = m.pB == null ? '' : m.pB;
...
function saveRemoteResult(m) {
  return new Promise(function(resolve, reject) {
    var cbName = 'copaSaveCallback_' + Date.now();
    window[cbName] = function(data) {
      try { if (data && data.ok) resolve(data); else reject(data || { error: 'Falha ao salvar' }); }
      finally {
        delete window[cbName];
        var el = document.getElementById(cbName);
        if (el && el.parentNode) el.parentNode.removeChild(el);
    ...
    script.id = cbName;
    script.src = SHEETS_API_URL + '?' + params;
    script.onerror = function() { delete window[cbName]; reject({ error: 'Erro de comunicação' }); };
    document.body.appendChild(script);
  });
}

function saveMatch() {
  if (editIndex < 0 || !matches[editIndex]) { alert('Nenhum jogo selecionado.'); return; }
  var m = matches[editIndex];
  var ga = document.getElementById('ga').value, gb = document.getElementById('gb').value;
  ...
  persist(); closeModal(); renderAll(); toast('Salvando na planilha...');
  saveRemoteResult(m).then(function() { toast('Resultado salvo na planilha.'); setTimeout(loadRemoteResults, 800); })
    .catch(function() { toast('Falha ao salvar na planilha.'); });
}

/* Render do chaveamento com lados corrigidos, sem chamadas circulares */
function renderBracketTree() {
  var target = document.getElementById('chave');
  if (!target) return;
  var names = knockoutNames();
  ...
  html += renderBracketSideV15(left, names, 'left-side');
  html += '<div class="tree-center tree-center-v15"><div class="final-box final-box-v15"><div class="tree-stage final-stage">Final</div>' + treeMatchV15(104, names) + '</div>';
  html += renderChampionBoxV15();
  html += '<div class="third-box third-box-v15"><div class="tree-stage">3º lugar</div>' + treeMatchV15(103, names) + '</div></div>';
  html += renderBracketSideV15(right, names, 'right-side');
  html += '</div></div><div class="small">A chave é preenchida automaticamente conforme os vencedores forem definidos.</div></div>';
  target.innerHTML = html;
  if (typeof setupBracketMobileZoom === 'function') setTimeout(setupBracketMobileZoom, 50);
}

/* ===== V28 RESUMO + AVANÇO DO MATA-MATA ===== */

function getMatchByNumV28(num){
  for(var i=0;i<matches.length;i++){ if(matches[i].num===num) return matches[i]; }
  return null;
}

function resolveRefV28(ref, seen){
  seen = seen || {};
  if(!ref) return '';
  if(typeof ref !== 'string') return ref;
  if(ref.charAt(0)!=='W' && ref.charAt(0)!=='L') return ref;
  ...
  var val = type==='W' ? winnerOfV28(num, Object.assign({}, seen)) : loserOfV28(num, Object.assign({}, seen));
  return val || (type==='W' ? 'Vencedor Jogo ' : 'Perdedor Jogo ') + num;
}

function namesForMatchV28(m, seen){
  if(!m) return {a:'',b:''};
  if(m.type==='ko'){
    return {
      a: resolveRefV28(m.aRef, Object.assign({}, seen || {})),
      b: resolveRefV28(m.bRef, Object.assign({}, seen || {}))
    };
  }
  return {a:m.a || '', b:m.b || ''};
}

function winnerOfV28(num, seen){
  seen = seen || {};
  if(seen['W'+num]) return null;
  seen['W'+num] = true;
  var m = getMatchByNumV28(num);
  if(!m || m.status !== 'encerrado' || m.gA == null || m.gB == null) return null;
  var nm = namesForMatchV28(m, seen);
  if(m.gA > m.gB) return nm.a;
  if(m.gB > m.gA) return nm.b;
  if(m.type==='ko' && m.pA != null && m.pB != null){
    if(m.pA > m.pB) return nm.a;
    if(m.pB > m.pA) return nm.b;
  }
  return null;
}

function loserOfV28(num, seen){
  seen = seen || {};
  if(seen['L'+num]) return null;
  seen['L'+num] = true;
  var m = getMatchByNumV28(num);
  if(!m || m.status !== 'encerrado' || m.gA == null || m.gB == null) return null;
  var nm = namesForMatchV28(m, seen);
  if(m.gA > m.gB) return nm.b;
  if(m.gB > m.gA) return nm.a;
  if(m.type==='ko' && m.pA != null && m.pB != null){
    if(m.pA > m.pB) return nm.b;
    if(m.pB > m.pA) return nm.a;
  }
  return null;
}

/* sobrescreve funções globais usadas pelo mata-mata */
function resolveRef(ref, seen){ return resolveRefV28(ref, seen || {}); }
function winnerOf(num, seen){ return winnerOfV28(num, seen || {}); }
function loserOf(num, seen){ return loserOfV28(num, seen || {}); }
function knockoutNames(){
  var out = {};
  for(var i=0;i<matches.length;i++){
    var m = matches[i];
    if(m.type !== 'ko') continue;
    out[m.num] = {a: resolveRefV28(m.aRef, {}), b: resolveRefV28(m.bRef, {})};
  }
  return out;
}

function todayTextV28(offset){
  var d = new Date();
...
copySummary = function(){
  var yesterday = todayTextV28(-1);
  var today = todayTextV28(0);
  var tabs = groupTables();
  var yesterdayGames = [];
  var impacted = {};
  var todayGames = [];
  ...
  for(var i=0;i<matches.length;i++){
    ...
    if(m.date===today && m.status!=='encerrado') todayGames.push(m);
  }
  ...
  if(yesterdayGames.length===0){
    ...
    var hasGroups=false;
    for(var groupId in impacted){ hasGroups=true; }
  ...
  if(todayGames.length===0){
    ...
    for(var j=0;j<todayGames.length;j++){
      var hj=todayGames[j], hn=namesForMatchV28(hj,{});
  ...
  var brazilToday=null;
  for(var b=0;b<todayGames.length;b++){
    var bn=namesForMatchV28(todayGames[b],{});
    if(bn.a==='Brasil' || bn.b==='Brasil'){ brazilToday={match:todayGames[b], names:bn}; break; }
  }
  ...
  if(brazilToday){
    ...
    var main=todayGames[0], mn=namesForMatchV28(main,{});
    txt += '🔥 Principal jogo do dia\n\n';
  ...
  if(navigator.clipboard && navigator.clipboard.writeText){
    navigator.clipboard.writeText(txt).then(function(){ toast('Resumo diário copiado.'); }, function(){ alert(txt); });
  }else alert(txt);
};

/* ===== V29 RESOLVEDOR ÚNICO DO MATA-MATA ===== */

function byNumV29(num) {
  for (var i = 0; i < matches.length; i++) {
...
function directNameV29(m, side) {
  if (!m) return '';
  if (m.type === 'group') return side === 'a' ? (m.a || '') : (m.b || '');
  return side === 'a' ? (m.aRef || '') : (m.bRef || '');
}

function resolvedNameV29(ref, stack) {
  stack = stack || {};
  if (!ref) return '';
  if (typeof ref !== 'string') return String(ref);

  var first = ref.charAt(0);
  if (first !== 'W' && first !== 'L') return ref;

  var num = parseInt(ref.substring(1), 10);
  if (isNaN(num)) return ref;

  var key = first + num;
  if (stack[key]) return first === 'W' ? 'Vencedor Jogo ' + num : 'Perdedor Jogo ' + num;
  stack[key] = true;

  var m = byNumV29(num);
  if (!m) return first === 'W' ? 'Vencedor Jogo ' + num : 'Perdedor Jogo ' + num;

  if (m.status !== 'encerrado' || m.gA == null || m.gB == null) {
    return first === 'W' ? 'Vencedor Jogo ' + num : 'Perdedor Jogo ' + num;
  }

  var a = resolvedNameV29(directNameV29(m, 'a'), Object.assign({}, stack));
  var b = resolvedNameV29(directNameV29(m, 'b'), Object.assign({}, stack));
  ...
  if (m.gA > m.gB) {
    ...
    if (m.pA > m.pB) { winner = a; loser = b; }
    else if (m.pB > m.pA) { winner = b; loser = a; }
  }

  if (first === 'W') return winner || 'Vencedor Jogo ' + num;
  return loser || 'Perdedor Jogo ' + num;
}
...
function namesV29(m) {
  ...
  if (m.type === 'ko') {
    return {
      a: resolvedNameV29(m.aRef, {}),
      b: resolvedNameV29(m.bRef, {})
    };
  }
  return { a: m.a || '', b: m.b || '' };
}
...
function winnerOf(num) {
  var name = resolvedNameV29('W' + num, {});
  return name.indexOf('Vencedor Jogo ') === 0 ? null : name;
}

function loserOf(num) {
  var name = resolvedNameV29('L' + num, {});
  return name.indexOf('Perdedor Jogo ') === 0 ? null : name;
}

function resolveRef(ref) {
  return resolvedNameV29(ref, {});
}
...
function knockoutNames() {
  ...
  for (var i = 0; i < matches.length; i++) {
    var m = matches[i];
    if (m.type !== 'ko') continue;
    out[m.num] = namesV29(m);
  }
  return out;
}

renderKnockout = function() {
  var target = document.getElementById('mata');
  if (!target) return;
...
openMatch = function(idx) {
  ...
  document.getElementById('mtitle').innerHTML = 'Jogo ' + m.num + ' · ' + (m.stage || ('Grupo ' + (m.group || ''))) + ' · ' + m.date + ' · ' + m.time;
  document.getElementById('fa').innerHTML = isRealTeam(nm.a) ? flagImg(nm.a, 'big') : '';
  document.getElementById('fb').innerHTML = isRealTeam(nm.b) ? flagImg(nm.b, 'big') : '';
  document.getElementById('na').innerHTML = nm.a;
  document.getElementById('nb').innerHTML = nm.b;
  document.getElementById('ga').value = m.gA == null ? '' : m.gA;
  document.getElementById('gb').value = m.gB == null ? '' : m.gB;
...
copySummary = function() {
  ...
  var brazilToday = null;
  for (var b = 0; b < todayGames.length; b++) {
    var bn = namesV29(todayGames[b]);
    if (bn.a === 'Brasil' || bn.b === 'Brasil') {
      brazilToday = { match: todayGames[b], names: bn };
      break;
    }
  }
...
var oldRenderAllV29 = renderAll;
renderAll = function() {
  oldRenderAllV29();
  renderKnockout();
  if (typeof renderBracketTree === 'function') renderBracketTree();
};
